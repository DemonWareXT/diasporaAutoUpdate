...
function wrapper()
{
	...
	window.d_autoupdater.setup = function()
	{
		...
		window.app.stream.autoupdate = function()
		{
			...
			window.app.stream.autoreload_on = true;
			/*switch to "autoupdate-mode". We force usage of the "newest posts" url instead of "load older posts" when scrolling down.*/
			window.app.stream.fetch();
			window.app.stream.autoreload_on = false;
			/*and switch back - the user may scroll the page to bottom and we need usual functionality.*/
		...
		window.app.stream.on('fetched', function()
		{
			setTimeout(function()
			{
				if (window.d_autoupdater.latest_entry !== null)
				{
					...
					if (newPostCount > 0)
					{
						/*Show the "show posts" button only if there are new posts that are currently hidden*/
						...
						{
							window.d_autoupdater.latest_entry.prevAll().css('display', '');
							/*Show old entries-*/
							window.d_autoupdater.latest_entry.css('border-top', '1px solid #3f8fba');
							/*Add horizontal bar to mark the position where new entries start*/
							if (window.d_autoupdater.last_marked_entry !== null)
							{
								window.d_autoupdater.last_marked_entry.css('border-top', '');
								/*Remote horizontal bar from old position*/
							}
							window.d_autoupdater.last_marked_entry = window.d_autoupdater.latest_entry;
							window.d_autoupdater.latest_entry = null;
							/*clear the entry so we can set it again some time*/

							$('#main_stream_refresh_button').remove();
							/*Remote "show posts" button - it has been clicked, there will be no posts to show now.*/
			...
			/*The whole thing is wrapped in a timeout. NOT ELEGANT. But we need to fire AFTER backbone.js has finished updating models and rendering the posts, so we can hide them again.*/

		});

		setInterval(window.app.stream.autoupdate, 20 * 1000);
		/*20 Seconds interval*/
		console.log("AutoUpdater installed, timer started.");
	};

	setTimeout(window.d_autoupdater.setup, 2000);
	/*Wait for javascript / backbone.js initialization to be over, before we hijack and install our scripts*/
